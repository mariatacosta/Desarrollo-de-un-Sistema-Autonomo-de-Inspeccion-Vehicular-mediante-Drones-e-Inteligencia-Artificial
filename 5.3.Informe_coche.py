...
# ======================
TARGET_CLASS = "car"
FRAME_SAVE_PATH = "5.3.InformeFinal/frames/"
MASK_SAVE_PATH = "5.3.InformeFinal/masks/"
DETECTION_SAVE_PATH = "5.3.InformeFinal/detections/"
METRICS_SAVE_PATH = "5.3.InformeFinal/metrics.csv"
DEPTH_MAPS_SAVE_PATH = "5.3.InformeFinal/depth_maps/"
MODELS_DETECTION_PATH = "5.3.InformeFinal/models_detections/"
...
SEGUNDA_ETAPA_DURACION = 350 # segundos
...
try:
    ...
    while time.time() - etapa2_start < SEGUNDA_ETAPA_DURACION:
                        ...
                        if depth_value > 0:
                            if PROFUNDIDAD_REFERENCIA is None:
                                ...
                                if abs(diferencia_relativa) > PROFUNDIDAD_TOLERANCIA:
                                    ...
                                    velocidad_frontal = int(np.clip(depth_error * 20, -20, 20))  # Factor de corrección
                                    print(
        ...
        if sofa_detected:
            ...
            cv2.imwrite(f"{MODELS_DETECTION_PATH}det_{frame_id:04d}.jpg", annotated_bgr)
    ...
    # ======================
    detections_csv_path = "5.3.InformeFinal/detection_metrics.csv"
    ...
    # Número de frames donde detectamos alguna parte/desperfecto
    num_frames = df_det["frame"].nunique()
    # Número de pares únicos defecto–parte
    ...
    for _, defect in defects_df.iterrows():
        ...
        for _, part in parts_df[parts_df["frame"] == frame_idx].iterrows():
            ...
            if ix2>ix1 and iy2>iy1:
                ...
                union = da + part["area"] - inter
                iou = inter/union if union>0 else 0
                if iou > max_iou:
                    max_iou, matched_part = iou, part["clase"]
        size_cm2 = da * (cal_cm_px**2)
        mappings.append({
    ...
    pdf.cell(0,8,f"Frames procesados con detecciones: {num_frames}",ln=1)
    pdf.cell(0,8,f"Pares únicos defecto-parte: {unique_df.shape[0]}",ln=1)
    ...
    for _, row in unique_df.iterrows():
        pdf.set_font("Arial", "B", 12)
        pdf.cell(0, 8, f"Frame {int(row['frame'])}: Desperfecto {row['defect_class']} - Parte {row['part_class']}",
                 ln=1)
    ...
    report_path = "5.3.InformeFinal/informe_deteccion.pdf"
