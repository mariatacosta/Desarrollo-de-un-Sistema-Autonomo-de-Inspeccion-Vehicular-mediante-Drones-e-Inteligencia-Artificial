...
# ======================
TARGET_CLASS = "couch"
FRAME_SAVE_PATH = "5.InformeFinal/frames/"
MASK_SAVE_PATH = "5.InformeFinal/masks/"
DETECTION_SAVE_PATH = "5.InformeFinal/detections/"
METRICS_SAVE_PATH = "5.InformeFinal/metrics.csv"
DEPTH_MAPS_SAVE_PATH = "5.InformeFinal/depth_maps/"
MODELS_DETECTION_PATH = "5.InformeFinal/models_detections/"
...
SEGUNDA_ETAPA_DURACION = 45  # segundos
...
PROFUNDIDAD_REFERENCIA = None  # se guardará la profundidad de calibración
...
try:
    ...
    # ======================
    detections_csv_path = "5.InformeFinal/detection_metrics.csv"
    ...
    # Estadísticas para la introducción
    creation_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    num_frames = int(df_metrics[df_metrics["sofa_detected"] == 1]["frame_id"].nunique())
    unique_parts = df_det[df_det["modelo"] == "parts"]["clase"].nunique()
    unique_defects = df_det[df_det["modelo"] == "defects"]["clase"].nunique() # Un ejemplo de cada clase
    calibration_value = PROFUNDIDAD_REFERENCIA if PROFUNDIDAD_REFERENCIA is not None else -1

    # Relacionar desperfectos con partes usando caja delimitadora
    parts_df = df_det[df_det["modelo"] == "parts"]
    defects_df = df_det[df_det["modelo"] == "defects"]
    mappings = []
    # Asumimos calibración de cm/px (puede ajustarse según necesidad)
    cal_cm_px = 0.17
    ...
    pdf.cell(0, 8, f"Número de frames detectados: {num_frames}", ln=1)
    pdf.cell(0, 8, f"Número de partes de coche únicas detectadas: {unique_parts}", ln=1)
    pdf.cell(0, 8, f"Número de desperfectos únicos detectados: {unique_defects}", ln=1)
    pdf.cell(0, 8, f"Valor de calibración (ProfundidadReferencia): {calibration_value:.2f}", ln=1)

    pdf.ln(10)
    # Tabla de desperfectos y partes
    pdf.set_font("Arial", "B", 12)
    th = pdf.font_size + 2
    pdf.cell(40, th, "Desperfecto", 1)
    pdf.cell(40, th, "Parte", 1)
    pdf.cell(30, th, "Confianza", 1)
    pdf.cell(40, th, "Tamaño (cm2)", 1)
    pdf.ln(th)
    pdf.set_font("Arial", "", 12)
    ...
    # Imágenes de detecciones
    pdf.add_page()
    for frame in map_df["frame"].unique():
        img_path = f"{MODELS_DETECTION_PATH}det_{frame:04d}.jpg"
        if os.path.exists(img_path):
            pdf.image(img_path, w=100)
            pdf.ln(5)

    report_path = "5.InformeFinal/informe_deteccion.pdf"
